...
var minimo_jugadores: int = 3
func check_iniciar_partida(res): 
	return forzar_inicio_partida or (res is Array and res.size() >= minimo_jugadores)

func espera_a_comienzo_partida(id: int, min_jugadores: int=3)->void:
	minimo_jugadores = min_jugadores
	if creado_partida:
		print("espero, he creado partida")
...
func mano(id: int, crea_ficha: Callable)->Array[Ficha]:
	await _awaiting_request_get(base_url+participaciones+"/"+str(mi_id)+"/"+str(id_partida))
	if _respuesta_buena():
		var campos = json.data
...
func subir_jugada(id:int, tablero: Array[Grupo_fichas]):
	var cadena_tablero: String = tablero.reduce(
		func(accum: String, grupo:Grupo_fichas)->String:
		var string_grupo: String = grupo.fichas.reduce(
			func(inner_accum:String, fich:Ficha)->String:
			return inner_accum + ficha_to_string(fich) + ","
			,"")
		return accum + string_grupo.left(-1) + ";"
		, "")
	print(cadena_tablero)
	printerr("debug, cambiar fecha")
	await _awaiting_request(base_url+partidas+"/"+str(id),
	{mesa:cadena_tablero.left(-1),fecha:"2026-03-31",partida_empezada:true,bolsa_robar:ultima_info_partida[bolsa_robar]},
	HTTPClient.METHOD_PUT,header())
	print(json.data)
